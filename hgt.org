...
#+begin_src R
  ...
  l <- readLAS("test2009.las", filter = "-drop_z_below 6 -thin_with_voxel 3 -keep_scan_angle -10 10")  # thin so that point density is constant?
...
#+RESULTS:
: Error in lapply(tiles.w.trees, function(tile) { : 
:   object 'tiles.w.trees' not found
