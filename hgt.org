...
#+PROPERTY:  header-args:R :cache no :results output :exports both :comments link :session *R:hgt* :eval no
...
HERE SHOWING INVENTORIES!!  give species breakdowns too.
...
#+begin_src R
  ...
  cl <- makeCluster(7)
  registerDoParallel(cl)
  out <- foreach(f = fl[-69][11:110], .packages = c("stringr","lidR")) %dopar% {  
      l <- readLAS(f)
      i <- str_extract(f, "[0-9]+_norm")
