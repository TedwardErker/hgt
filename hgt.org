...
sequestration cite:roy_etal_2012.  
...
the 2015-2016 inventory of the HillFarms neighborhood on Madison's
...
squared to an average of XXXX pulse per meter squared.
...
from lidar to mask any 4.6 pixels that had a mean height less than 8
...
=raster= cite:raster_Rpackage.  The DEM was originally at
...
assuming that other than pulse density all else was equal across lidar
...
figures [[fig:growthrate_byheight_byneighst]], [[fig:impervious_interactions]]  below along with explanation.
...
elevation might correspond to soils

what does the aspect transformation correspond to?  NE
...
growth with multiple years of lidar.  Second, we show that, as for all
trees, urban tree height growth is significantly related to their
...
dwellers with both services and disservices.  Our work has shown that
the growth of trees - and the growth related (dis)services -  may in turn
be affected by the urban environment designed by humans, particularly
impervious cover.  Trees are dynamic parts of the urban ecosystem and
their function can vary considerably, meaning managers cannot simply
plant a tree and expect it to provide a certain benefit without
considering the tree's context.  
