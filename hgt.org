...
* Phil's comments on the pdf on the first few pages of pdf, then on the google doc.
- add a supplementary table for each genera's bias correction at 0.25,
  .5, .75, 1,  pulses per m^2
- study area map
- Madison
  - aviris flight lines.
- tree inventories
...
cite:nowak14_tree_fores_effec_air_qualit,nowak_e_2013. Therefore,
...
different than its rural surrounding cite:zipperer_guntenspergen_2009.
Urban conditions are not only unique, but they are also more
heterogeneous, which creates variability in the growth of the urban
forest at small spatial scales.
...
can be used to create indicies of canopy foliar traits which then in
turn we used to explain height growth.
...
| neighborhood or street tree with or without overhead utilities | tree inventories                 |
...
For the purposes of estimating coefficients more reliably, we subset
our data so that no two trees of the same genus were closer than 200 m
to one another.  This reduced our sample size from about 21 thousand
to 7 thousand, but also reduced spatial dependence that could
potentially bias coefficient estimates or inflate confidence that
coeffient estimates are different from zero.  We fit several multiple
linear regressions with growth rate as a response and all covariates
as predictors with various interactions that were reasonable given our
objectives.  We used AIC to help with model selection, but did not
test very many interactions, instead we were primarily guided by our
objectives and the goal of finding a model that was straightforward to
communicate.  Details on the selected models are in results.
...
The mean height growth rate of the 7 thousand trees in our sample was
...
cm/yr) suggest there is considerable measurement error in
growth rate.
...
parameters) and a higher adjusted R^2 (.569 compared to .506).  There
...
neighborhood trees were growing at an estimated 35 cm / year and
street trees at an estimated 28 cm / year.  Depending on the type of
tree, asympotitc heights are different.  For street trees it occurred
at heights around 25 m; for neighborhood trees: 27 m.
...
growth rates, as low as 2.5 cm / year.  But trees with low impervious
cover locally were able to maintain high growth rates despite having
over 50% impervious cover within 100m.  This situation might occur for
trees in small parks or yards surrounded by built up surfaces. Growth
...
growing in the highest parts of the city grew about 2.2 cm / year more
...
surroundings they grew an estimated 2.5 cm less per year.  Slope was
not significant at the 61 m scale, but at the 1.5 m scale trees on
steeper slopes grew more slowly than trees in flatter regions.
Surprisingly, aspect had opposite effects depending on the scale: at
the 1.5 m scale trees in the northeast aspect grew faster than trees
in the southwest aspect by almost .75 cm / year, while at the 61 m
scale the effect was nearly opposite.
...
they increase the adjusted R^2 from .12 to .50 for a model without genus
...
Catalpa and Carya (estimated marginal growth rates over 17.5 cm / year).
...
had greater bias.  This relationship warrants more rigorous assessement.
...
Some of the residual spatial structure in the data could be explained
...
for trees on private property.  The foliar canopy traits show promise
in improving canopy height predictions when genus is not known.
...
significant effects, though the effect of topography was relatively small.
...
could maintain relatively fast growth.  This could possibly be because
trees had sufficient soil locally and may have been receiving
increased water from runoff or were responding to the elevated
temperatures in the broader surroundings.
...
immediate surroundings all grew more than trees in the opposite
conditions.  Why trees at higher elevations grew more than trees at
lower elevations is not clear, but could be due to another factor that
varies from the relatively lower eastern half of the city to the
relatively higher western half of the city, such as soils.  Aspect had
opposite effects depending on the scale.  Trees in general tend to
grow better in the cooler and wetter northeastern aspects.  Madison,
however, does not have much topography and it is also a city.  It
could be that the expected aspect growth relationship is disrupted in
a city without a large variation in topography.
...
but there are several areas for improvement and future work.  First,
while better than doing nothing, our height bias correction method
could be improved by considering more characteristics of the lidar
that lead to bias.  Second, we only consider tree height growth,
rather than some more ecologically relevant like biomass growth.
Height growth is much easier to estimate from lidar, but provides an
incomplete picture, especially for taller trees or more open grown,
which may still be growing but not allocating resources to height.

Third, our subsampling of the data to remove trees of the same genus
that were closer than 200m to each other was a coarse technique to
reduce spatial dependence in our data.  Properly modeling the
covariance structure of our observations could better control for this
dependence.  When we fit the model with all 21 thousand available
observations, we found residuals of the model had strong spatial
structure.  There clearly remain important factors in the urban
environment that we are missing in our model (e.g. soils).
...
All the bias correction values used to adjust heights based on pulse
density are available in the R binary file: =no_growth_bias.rds= in
the paper's github repository.
| Genus       | number of pulses | pulse density (m^-2) | mean bias (m) | mean standard deviation of bootstrap bias (m) |
|-------------+------------------+---------------------+---------------+-----------------------------------------------|
| Abies       |                2 |                 0.1 |         -2.23 |                                          1.85 |
| Abies       |                9 |                 0.5 |          -0.7 |                                          0.53 |
| Abies       |               15 |                 0.8 |         -0.48 |                                          0.38 |
