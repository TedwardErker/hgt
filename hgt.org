...
Urban trees grow differently than their counterparts because of the
challenges they face surviving among urban infrastructure and are
cultivated by humans (but not for their wood).  For example, street
trees are subject to damage from vehicles and pedestrians; urban trees
tend to be open grown; urban trees are pruned to accommodate
powerlines or for aesthetics, many urban trees are irrigated and
fertilized; and the species composition in a city is often different
cite:zipperer_guntenspergen_2009.  Urban conditions are not only
...
and then help explain patterns in that growth for tens of thousands of
trees.  Sufficient sample size is required to detect effect sizes that
may be small relative to the environment.  Here we use two
technologies that have only recently become more available, repeat
airborne laser scanning (LiDAR, also called light detection and
...
characterize forest structure, including height cite:lim_e_2003.
...
bias correction, to estimate the heights of trees and their growth
rates.
...
Again, you are lacking a literature review.  Just a few sentences are
needed, but tell us what spectroscopy is, how it has been used to do
these things, and what the variables derived from it tell you about
ecosystems (investment in growth potential vs leaf construction,
defense) 
...
or not it had overhead utilities. 

Like with hyperspectral and lidar, you need a sentence lit review (w/
citations) on each of these variables and why you would hypothesize
they affect tree growth
...
** cite:duncanson18_monit_indiv_tree_based_chang
