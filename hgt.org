...
- Percent impervious cover interacts at multiple scales to affect tree
  height growth: trees with low imperivious cover within 20m and high
...
factors therby altering the growth of trees.
...
lidar and imaging spectroscopy.  Lidar uses the reflection of lazer
pulses to create 3 dimensional point clouds of objects.  Using
multiple years of lidar data, in conjuction with an innovative height
bias correction, we estimated the heights of trees and their growth
rates.  NASA's Airborne Visible / Infrared Imaging Spectometer - Next
...
taxonomic infomation of trees.  In September 2015, NASA's Airborne
Visible / Infrared Imaging Spectometer - Next Generation (AVIRIS-NG)
...
(% mass), chlophyll (mmol / m^2), and total phenolics (% mass).  We
...
| elevation (hundredss of meters)                                      | DEM                              |
...
Spectometer - Next Generation (AVIRIS-NG) collected imagery over the
...
area (g/m^2), LMA (g / m^2); lignin (% mass), chlophyll (mmol / m^2), and
...
togther.  Taxa with thicker twigs will have stronger returns, but
...
else was equal across lidar aquisitions.  We then sampled 1000 times
...
growth rate was 8.7 cm per year.  We only analysed in depth the bias
...
#+caption: Histogram of estimated growth rates for nearly 42,000 urban trees.  The sample mean growth rate is 12.8 cm / year.
[[file:figs/growthrate_histogram_w.png]]
...
utilities overhead it occured at heights around 24 m; for street trees
...
were supressed or overtopped.  

Figure [[fig:mean_height_20m_coefs]] shows coeffiecient estimates with 95%
...
creates challenges in using multitemporal lidar to estimate forest
...
height for multitemporal lidar, but also within single date lidar
...
Most of the coefficicients of foliar canopy traits were in expected
...
the model does not affect the direction of the nitrogen coefficeint,
...
second biological explantion could be that nitrogen deposition is
...
negative assocation of nitrogen with growth does not appear to be a
...
association with growth rates.  Overtopped and suppressed trees were
