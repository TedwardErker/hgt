...
- Four foliar canopy traits predicted from imaging spectroscopy:
  ...
  coefficients.  Traits contribute significantly to the model that
  does not include genus.
...
contribute to the growth of trees are well understood, thanks to
hundreds of years of forestry and plant biology.  What we do not
...
four years of lidar data (2005, 2009, 2016, and 2017) and an innovative
height bias correction method, we estimated the height growth of trees
over a 12 year period.  We used two street tree inventories (one from
the city of Madison and one from the village of Shorewood, which is
within the city) and two complete inventories of the trees Madison's
Atwood and Hill Farms neighborhoods to get taxonomic infomation of
trees.  In September 2015, NASA's Airborne Visible / Infrared Imaging
Spectometer - Next Generation (AVIRIS-NG) imaged the majority of
Madison.  Using predictive equations developed following
cite:singh_e_2015, we predicted canopy foliar traits: Nitrogen (%
mass), N; leaf mass per area (g/m^2), LMA; lignin (% mass), chlophyll,
and total phenolics.  We converted these trait predictions to an index
because of imagery differences between our site and where the model
was built.  In addition to foliar traits, we calculated other
covariates such as percent impervious cover at two scales (within 20m
and within 100m), topographic variables, and the mean height of the
surroundings within 20 m of a tree (see table [[tab:covariates]] for all
covariates).
...
** Data
*** Tree inventories
The genus of a tree encodes a suite of genetic factors that affect
growth rates and is therefore important to know when modeling tree
growth.  We used four tree inventories for species information.  The
first is the city of Madison's street tree inventory from 2011; the
second is the 2012 inventory of the Atwood neighborhood located on
...
area (g/m^2), LMA; lignin (% mass), chlophyll, and total phenolics.  We
converted these trait predictions to an index because of imagery
differences between our site and where the model was built meant that
predictions were biased and not quite within biologically reasonable
ranges.  To create this index we adjusted traits predictions across
images using overlap pixels to be consistent across images and then
standardized the traits by dividing by their standard deviation.
...
We are missing information on footprint size....
...
They matter.  Especially if we don't know Genus, they appear useful.
