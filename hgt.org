...
#+begin_src R :exports results :results graphics :file figs/tenney_oak1.png :width 1500 :height 500 :res 120 :bg transparent
...
#+begin_src R :exports results :results graphics :file figs/tenney_oak1_firstreturns.png :width 1500 :height 500 :res 120 :bg transparent

  ggplot(filter(d, ReturnNumber ==1), aes(x = X - mean(X), y = Z, color = year)) +
      geom_point(size = .5) +
      coord_equal() +
      facet_wrap(~year, ncol = 4) +
    terk +
...
#+begin_src R :exports results :results graphics :file figs/tenney_oak_crosssection.png  :width 1500 :height 500 :res 120 :bg transparent

#    xc <- 825410

    dc <- filter(d, abs(X - mean(X)) < 8 * .3048)
    ggplot(dc, aes(x = Y - mean(Y), y = Z, color = year)) +
        geom_point(size = .5) +
        coord_equal() +
        facet_wrap(~year, ncol = 4) +
    terk +
...
#+begin_src R :exports results :results graphics :file figs/tenney_oak_crosssection_vline.png  :width 1500 :height 500 :res 120 :bg transparent
    dc <- filter(d, abs(X - mean(X)) < 8 * .3048)
    ggplot(dc, aes(x = Y - mean(Y), y = Z, color = year)) +
       geom_vline(xintercept = 8 * .3048) +
       geom_vline(xintercept =  -8 * .3048) +
        geom_point(size = .5) +
        coord_equal() +
        facet_wrap(~year, ncol = 4) +
...
#+begin_src R :exports results :results graphics :file figs/tenney_oak_crosssection_vline_centercolumn.png  :width 1500 :height 500 :res 120 :bg transparent
    dc <- filter(d, abs(X - mean(X)) < 8 * .3048)
    dcc <- filter(dc, abs(Y - mean(Y)) < 8 * .3048)
    ggplot(dc, aes(x = Y - mean(Y), y = Z, color = year)) +
       geom_vline(xintercept = 8 * .3048) +
       geom_vline(xintercept =  -8 * .3048) +
        geom_point(size = .5, alpha = .08) +
...
#+begin_src R :exports results :results graphics :file figs/tenney_oak_centerpoints.png :width 700 :height 600 :res 120 :bg transparent
dcc <- filter(dcc, Z > 2)
    ggplot(dcc, aes(x = Y - mean(Y), y = Z, color = year)) +
        geom_point(size = 1) +
        coord_equal() +
        facet_wrap(~year, ncol = 4) +
    terk +
  scale_y_continuous("Height (m)", limits = c(0,20)) +
  scale_x_continuous("Distance from center (m)") +
    scale_color_brewer(palette = "Dark2")
#+end_src
...
#+begin_src R :exports results :results graphics :file figs/tenney_oak_centerpoints_maxheightlines.png :width 700 :height 600 :res 120 :bg transparent
  dcc <- filter(dcc, Z > 2)

  dccs <- dcc %>%
    group_by(year) %>%
    summarize(maxh = max(Z))

dccp <- left_join(dcc, dccs)

      ggplot(dccp, aes(x = Y - mean(Y), y = Z, color = year)) +
          geom_point(size = 1) +
          coord_equal() +
          facet_wrap(~year, ncol = 4) +
      terk +
    ...
    scale_x_continuous("Distance from center (m)") +
      scale_color_brewer(palette = "Dark2") +
      geom_hline(aes(yintercept = max(dcc$Z))) +
      geom_hline(aes(yintercept = maxh, color = year))

#+end_src

#+RESULTS:
[[file:figs/tenney_oak_centerpoints_maxheightlines.png]]

#+begin_src R :exports results :results graphics :file figs/tenney_oak_centerpoints_maxheightlines_resampled1.png :width 700 :height 600 :res 120 :bg transparent
#set.seed(4)
dccn <- mutate(dcc, year = sample(year, replace = F))

  dccs <- dccn %>%
    group_by(year) %>%
    summarize(maxh = max(Z))

dccp <- left_join(dccn, dccs)
...
#+RESULTS:
[[file:figs/tenney_oak_centerpoints_maxheightlines_resampled1.png]]
...
p <-       ggplot(dccp, aes(x = Y - mean(Y), y = Z, color = year)) +
          geom_point(size = 1) +
          coord_equal() +
          facet_wrap(~year, ncol = 4) +
      terk +
...
png(sprintf("./figs/resample/%05d.png", i), width = 700, height = 600, res = 120, bg = "transparent")
print(p)
dev.off()
...
#+begin_src R
  ...
  l2017 <- readLAS("/home/erker/hgt_data/madison_lidar_2017_heights/normalized/0824_esri_normalized.las", select = "r")
    ...
    p <- shapefile("/home/erker/hgt_data/example_trees/ST69002.shp")
...
#+begin_src R
#  plot(o2016,  bg = "white", colorPalette = "black", size = 7)
  plot(o2016,  bg = "white", size = 8)
#+end_src
...
: 
:              [,1]        [,2]        [,3] [,4]
: [1,]  0.990829825  0.13425344  0.01523324    0
: [2,] -0.003196636 -0.08941901  0.99598897    0
: [3,]  0.135077089 -0.98690438 -0.08816990    0
: [4,]  0.000000000  0.00000000  0.00000000    1
...
: [1] 360
: 
: [1] "/home/erker/git/hgt/animation"
#+BEGIN_SRC sh :session a
cd ~/git/hgt/animation/
convert -delay 8 -loop 1 *.png ../figs/maple_withhole_small.gif
